 ...
 */
function calculateVolatilityThresholds(volatilities: number[]): [number, number, number, number] {
  // Filter out zero volatility and sort
  const validVolatilities = volatilities.filter(v => v > 0).sort((a, b) => a - b);
  ...
  const p20 = validVolatilities[Math.floor(validVolatilities.length * 0.2)] || validVolatilities[0];
  const p40 = validVolatilities[Math.floor(validVolatilities.length * 0.4)] || validVolatilities[0];
  const p60 = validVolatilities[Math.floor(validVolatilities.length * 0.6)] || validVolatilities[0];
  const p80 = validVolatilities[Math.floor(validVolatilities.length * 0.8)] || validVolatilities[validVolatilities.length - 1];
 ...
 * - very_hot: [threshold4 - ...)
 */
function calculateVolatilityBand(volatility: number, thresholds: [number, number, number, number]): VolatilityBandId {
...
export function transformClientData(rawData: RawAssetData[]): {
  ...
  console.log('📊 Volatility thresholds (data-driven):', {
    ...
    mild: `[${volatilityThresholds[0].toFixed(3)} - ${volatilityThresholds[1].toFixed(3)})`,
    warm: `[${volatilityThresholds[1].toFixed(3)} - ${volatilityThresholds[2].toFixed(3)})`,
    hot: `[${volatilityThresholds[2].toFixed(3)} - ${volatilityThresholds[3].toFixed(3)})`,
  ...
  assetMap.forEach((data, assetName) => {
    ...
    const volatilityBand = calculateVolatilityBand(data.volatility, volatilityThresholds);
